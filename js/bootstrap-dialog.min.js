(function(a,b){if(typeof module!=="undefined"&&module.exports){module.exports=b(require("jquery")(a));}else{if(typeof define==="function"&&define.amd){define("bootstrap-dialog",["jquery"],function(c){return b(c);
...
if(typeof d.fn.modal.Constructor.VERSION==="undefined"){e="v3.1";}else{if(/3\.2\.\d+/.test(d.fn.modal.Constructor.VERSION)){e="v3.2";}else{e="v3.3";}}return e;
};c.ORIGINAL_BODY_PADDING=d("body").css("padding-right")||0;c.METHODS_TO_OVERRIDE={};c.METHODS_TO_OVERRIDE["v3.1"]={};c.METHODS_TO_OVERRIDE["v3.2"]={hide:function(g){if(g){g.preventDefault();
...
this.$element.trigger("hidden.bs.modal");},this));}};c.prototype={constructor:c,getGlobalOpenedDialogs:function(){var e=[];d.each(a.dialogs,function(g,f){if(f.isRealized()&&f.isOpened()){e.push(f);
}});return e;}};c.prototype=d.extend(c.prototype,b.prototype,c.METHODS_TO_OVERRIDE[c.getModalVersion()]);var a=function(e){this.defaultOptions=d.extend(true,{id:a.newGuid(),buttons:[],data:{},onshow:null,onshown:null,onhide:null,onhidden:null},a.defaultOptions);
...
this.holdThisInstance();};a.NAMESPACE="bootstrap-dialog";a.TYPE_DEFAULT="type-default";a.TYPE_INFO="type-info";a.TYPE_PRIMARY="type-primary";a.TYPE_SUCCESS="type-success";
a.TYPE_WARNING="type-warning";a.TYPE_DANGER="type-danger";a.DEFAULT_TEXTS={};a.DEFAULT_TEXTS[a.TYPE_DEFAULT]="Information";a.DEFAULT_TEXTS[a.TYPE_INFO]="Information";
a.DEFAULT_TEXTS[a.TYPE_PRIMARY]="Information";a.DEFAULT_TEXTS[a.TYPE_SUCCESS]="Success";a.DEFAULT_TEXTS[a.TYPE_WARNING]="Warning";a.DEFAULT_TEXTS[a.TYPE_DANGER]="Danger";
a.DEFAULT_TEXTS.OK="OK";a.DEFAULT_TEXTS.CANCEL="Cancel";a.SIZE_NORMAL="size-normal";a.SIZE_SMALL="size-small";a.SIZE_WIDE="size-wide";a.SIZE_LARGE="size-large";
a.BUTTON_SIZES={};a.BUTTON_SIZES[a.SIZE_NORMAL]="";a.BUTTON_SIZES[a.SIZE_SMALL]="";a.BUTTON_SIZES[a.SIZE_WIDE]="";a.BUTTON_SIZES[a.SIZE_LARGE]="btn-lg";
a.ICON_SPINNER="glyphicon glyphicon-asterisk";a.defaultOptions={type:a.TYPE_PRIMARY,size:a.SIZE_NORMAL,cssClass:"",title:null,message:null,nl2br:true,closable:true,closeByBackdrop:true,closeByKeyboard:true,spinicon:a.ICON_SPINNER,autodestroy:true,draggable:false,animate:true,description:""};
...
this.updateZIndex();this.setOpened(true);return this;}};a.METHODS_TO_OVERRIDE["v3.2"]={handleModalBackdropEvent:a.METHODS_TO_OVERRIDE["v3.1"]["handleModalBackdropEvent"],updateZIndex:a.METHODS_TO_OVERRIDE["v3.1"]["updateZIndex"],open:a.METHODS_TO_OVERRIDE["v3.1"]["open"]};
a.METHODS_TO_OVERRIDE["v3.3"]={};a.prototype={constructor:a,initOptions:function(e){this.options=d.extend(true,this.defaultOptions,e);return this;},holdThisInstance:function(){a.dialogs[this.getId()]=this;
return this;},initModalStuff:function(){this.setModal(this.createModal()).setModalDialog(this.createModalDialog()).setModalContent(this.createModalContent()).setModalHeader(this.createModalHeader()).setModalBody(this.createModalBody()).setModalFooter(this.createModalFooter());
...
return this;},createModal:function(){var e=d('<div class="modal" tabindex="-1" role="dialog" aria-hidden="true"></div>');e.prop("id",this.getId()).attr("aria-labelledby",this.getId()+"_title");
...
}else{this.getModalFooter().find("."+this.getNamespace("footer")).html("").append(this.createFooterButtons());}}return this;},isAutodestroy:function(){return this.options.autodestroy;
...
e.indexedButtons[h.id]=i;f.append(i);});return f;},createButton:function(e){var f=d('<button class="btn"></button>');f.prop("id",e.id);if(typeof e.icon!=="undefined"&&d.trim(e.icon)!==""){f.append(this.createButtonIcon(e.icon));
}if(typeof e.label!=="undefined"){f.append(e.label);}if(typeof e.cssClass!=="undefined"&&d.trim(e.cssClass)!==""){f.addClass(e.cssClass);}else{f.addClass("btn-default");
}if(typeof e.hotkey!=="undefined"){this.registeredButtonHotkeys[e.hotkey]=f;}f.on("click",{dialog:this,$button:f,button:e},function(i){var h=i.data.dialog;
var j=i.data.$button;var g=i.data.button;if(typeof g.action==="function"){g.action.call(j,h);}if(g.autospin){j.toggleSpin(true);}});this.enhanceButton(f);
...
return this;},isOpened:function(){return this.opened;},setOpened:function(e){this.opened=e;return this;},handleModalEvents:function(){this.getModal().on("show.bs.modal",{dialog:this},function(f){var e=f.data.dialog;
if(e.isModalEvent(f)&&typeof e.options.onshow==="function"){return e.options.onshow(e);}});this.getModal().on("shown.bs.modal",{dialog:this},function(f){var e=f.data.dialog;
e.isModalEvent(f)&&typeof e.options.onshown==="function"&&e.options.onshown(e);});this.getModal().on("hide.bs.modal",{dialog:this},function(f){var e=f.data.dialog;
if(e.isModalEvent(f)&&typeof e.options.onhide==="function"){return e.options.onhide(e);}});this.getModal().on("hidden.bs.modal",{dialog:this},function(f){var e=f.data.dialog;
e.isModalEvent(f)&&typeof e.options.onhidden==="function"&&e.options.onhidden(e);e.isAutodestroy()&&d(this).remove();a.moveFocus();});this.handleModalBackdropEvent();
this.getModal().on("keyup",{dialog:this},function(e){e.which===27&&e.data.dialog.isClosable()&&e.data.dialog.canCloseByKeyboard()&&e.data.dialog.close();
...
return this;},open:function(){!this.isRealized()&&this.realize();this.getModal().modal("show");this.setOpened(true);return this;},close:function(){if(this.isAutodestroy()){delete a.dialogs[this.getId()];
}this.getModal().modal("hide");this.setOpened(false);return this;}};a.prototype=d.extend(a.prototype,a.METHODS_TO_OVERRIDE[c.getModalVersion()]);a.newGuid=function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(g){var f=Math.random()*16|0,e=g==="x"?f:(f&3|8);
return e.toString(16);});};a.show=function(e){return new a(e).open();};a.alert=function(){var f={};var e={type:a.TYPE_PRIMARY,title:null,message:null,closable:true,buttonLabel:a.DEFAULT_TEXTS.OK,callback:null};
...
}return new a({type:f.type,title:f.title,message:f.message,closable:f.closable,data:{callback:f.callback},onhide:function(g){!g.getData("btnClicked")&&g.isClosable()&&typeof g.getData("callback")==="function"&&g.getData("callback")(false);
...
g.close();}}]}).open();};a.confirm=function(e,f){return new a({title:"Confirmation",message:e,closable:false,data:{callback:f},buttons:[{label:a.DEFAULT_TEXTS.CANCEL,action:function(g){typeof g.getData("callback")==="function"&&g.getData("callback")(false);
g.close();}},{label:a.DEFAULT_TEXTS.OK,cssClass:"btn-primary",action:function(g){typeof g.getData("callback")==="function"&&g.getData("callback")(true);
g.close();}}]}).open();};a.warning=function(e,f){return new a({type:a.TYPE_WARNING,message:e}).open();};a.danger=function(e,f){return new a({type:a.TYPE_DANGER,message:e}).open();
